...
#PLAYER/SPELL/WEAPON CREATE/SAVE/LOAD METHODS
def getPlayerStats(data):
    ...
    player = Player(playerName, playerArray, playerAC, playerHP, class_type,
                    playerLvl, conImmunities, damImmunes, damResists, damVulns, activeStatusEffects, activeConditions)
    return player
...
def addChosenSpell(spell, player):
        ...
        if isinstance(spell["targeting"], list): #Multiple possible effects
...
def loadEncounter(encounterIdx):
    ...
    for monsterJSON in encounterData["monsters"]:
        ...
        legAction = bool(monsterJSON["legAction"])
        enemy = bool(monsterJSON["enemy"])
        encounter.addMonster(Monster(name, cr, cType, stats, hp, maxHP, ac, saveProfs, lResists,
                                     damResists, damImmunes, damVulns, conImmunes, activeConditions, activeStatusEffects,
                                     lairAction, legAction, enemy))
...
def saveEncounter(encounter, filename, autosave=False):
    ...
    for i in range(encounter.playerSize()):
        stats_dict = {
            ...
            "level": str(encounter.getPlayer(i).getLevel()),
            "ac": str(encounter.getPlayer(i).getAC()),
            "hp" : str(encounter.getPlayer(i).getHP()),
            "maxHP": str(encounter.getPlayer(i).getMaxHP()),
            "class": encounter.getPlayer(i).getClass(),
            "conImmunities": encounter.getPlayer(i).getConImmunities(),
            "damImmunes": encounter.getPlayer(i).getDamImmunities(),
            "damResists": encounter.getPlayer(i).getDamResistances(),
            "damVulns": encounter.getPlayer(i).getDamVulnerabilities(),
            "activeStatusEffects": encounter.getPlayer(i).getActiveStatusEffects(),
            "activeConditions": encounter.getPlayer(i).getActiveConditions(),
            "statArray": [str(encounter.getPlayer(i).getStat(stat)) for stat in ["STR", "DEX", "CON", "INT", "WIS", "CHA"]]
        }

        spells_list = []
        for j in range(encounter.getPlayer(i).getSpellLength()):
            spells_list.append(encounter.getPlayer(i).getSpell(j).toDict())

        weapons_list = []
        for j in range(encounter.getPlayer(i).getWeaponLength()):
            weapons_list.append(encounter.getPlayer(i).getWeapon(j).toDict())
...
def createEncounter():
    ...
    while monChoice != "Exit" or numAdded == 0:
        ...
        if found:
            ...
            cr = monsters[idx]["cr"]
            creatureType = monsters[idx]["creatureType"]
            stats = monsters[idx]["statArray"]
            hp = monsters[idx]["hit_points"]
            maxHP = hp
            ac = monsters[idx]["AC"]
            saves = monsters[idx]["saveProfs"]
            ...
            lairActions = False if monsters[idx]["lairAction"] == "False" else True
            legActions = False if monsters[idx]["legAction"] == "False" else True
            enemy = True

            encounter.addMonster(Monster(monChoice, cr, creatureType, stats, hp, maxHP, ac, saves,
                              lResists, damResists, damImmunes, damVulns,
                              conImmunes, [], [], lairActions, legActions, enemy))
...
def setInitiative(encounter):
    ...
    for i in range(encounter.monsterSize()):
        ...
        if encounter.getMonster(i).hasLairAction():
            ...
            #     'currentTurn': False
            # })
            pass #TODO: Temporarily removing for simplified pytest logic.
    ...
    if "cond" in effect:
        ...
        if isinstance(effect["effect"]["resultID"], list):
            ...
            if effect["effect"]["resultID"] == -1:
                return True
            return False
def enemyTurn(initiative, encounter):
...
def translateBasicAction(player, action):
        ...
        if "damageMod" in targeting:  # Accounts for schema error
            if actionRolls["damageMod"] == "spellmod":
                damageMod = player.getSpellMod()
...
def translateLingEffect(action, lingEffect, spellMod):
    ...
    lingSpell = Spell(
        ...
        damMod, lingDieNum, lingDieType, lingEffect["damType"],
...
def resolve_summon_spell(encounter, player, spell, initiative_list):
    ...
    if len(cr_caps) > 1:
        ...
        while choice_idx < 1 or choice_idx > len(cr_caps):
            #TODO: Needs input validation to make sure a chosen CR is actually in the list of options.
            try:
    ...
    if total_to_summon <= 0: #TODO: Shouldn't be possible.
    ...
    #TODO: Is getting a ValueError every time when looking from EncounterRunnerV2.
    try:
    ...
    while made < total_to_summon:
        ...
        legActions = False if chosen["legAction"] == "False" else True
...
def endSpellEffect(effect, idx, creature, initiative):
    ...
    #TODO: Make sure that this works with conditions to pull effectID, and end conditions.
    if "effect" in effect:
...
def preTurnCheck(creature, encounter, initiative):
    def procPreEffect(lingSpell):
        # HP logic
        if lingSpell["targeting"]["rolls"]["damage"]:
            damage = int(input(f"{lingSpell["spellname"]} - How much damage was applied?"))
        else:
            damage = 0
        if lingSpell["targeting"]["extraEffect"]:
            if lingSpell["targeting"]["extraEffect"]["rolls"]["damage"]:
                damage += int(input("EXTRA EFFECT\nHow much damage was applied?"))
        damType = lingSpell["targeting"]["damType"] if "damType" in lingSpell["targeting"] and \
                                                       lingSpell["targeting"]["damType"] else ""
    ...
    #TODO: Test turnCount with multiple attributes.
    for effect in creature.getActiveStatusEffects():
        if effect["name"].lower() in ["lingsave", "lingeffect"]:
    ...
    if preEffects:
        for effect in preEffects:
            ...
            for result in preTurnLogs: #Result log all preTurnEffects.
                #TODO: Test lingEffect result logging.
